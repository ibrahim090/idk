...
// firebaseConfig loaded from firebase-config.js
...
// --- TEMP: Seed White PC ---
window.seedWhitePC = async () => {
    if (!confirm("Add 'White Aesthetic Gaming PC' products to inventory?")) return;
    ...
    const products = [
        {
            ...
            category: "cpu",
            socket: "AM5",
            image_url: "https://m.media-amazon.com/images/I/51f2hk81eGL._AC_SL1000_.jpg",
            description: "6-Core, 12-Thread Unlocked Desktop Processor",
            specs: ["AM5 Socket", "Max Boost 5.1GHz", "65W TDP"]
        },
        {
            ...
            socket: "AM5",
            image_url: "https://m.media-amazon.com/images/I/81M6bm6VaaL._AC_SL1500_.jpg",
            description: "White themed AM5 Motherboard with WiFi 6E",
            specs: ["AM5 Socket", "DDR5 Support", "White PCB Accents"]
        },
        {
            name: "Corsair Vengeance RGB DDR5 32GB White",
            price: 129,
            stock: 20,
            category: "ram",
            image_url: "https://m.media-amazon.com/images/I/61+7w+1yjlL._AC_SL1500_.jpg",
            description: "32GB (2x16GB) DDR5 6000MHz C36 Memory Kit - White",
            specs: ["DDR5", "6000MHz", "RGB Lighting", "White Heatspreader"]
        },
        {
            ...
            stock: 8,
            category: "gpu",
            image_url: "https://m.media-amazon.com/images/I/71Y-3i-kLTL._AC_SL1500_.jpg",
            description: "8GB GDDR6, 128-bit, White Graphics Card",
            specs: ["8GB VRAM", "White Design", "Triple Fan"]
        },
        {
            name: "WD_BLACK SN770 1TB NVMe Gen4",
            price: 79,
            stock: 15,
            category: "storage",
            image_url: "https://m.media-amazon.com/images/I/614-727c6pL._AC_SL1500_.jpg",
            description: "High performance Gen4 PCIe M.2 SSD",
            specs: ["1TB", "Gen4", "Up to 5150 MB/s"]
        },
        {
            name: "Corsair RM750 White 80+ Gold",
            price: 109,
            stock: 10,
            category: "psu",
            image_url: "https://m.media-amazon.com/images/I/71c6w5tXf8L._AC_SL1500_.jpg",
            description: "Fully Modular White Power Supply",
            specs: ["750W", "80+ Gold", "White Cables Included"]
        },
        {
            ...
            stock: 12,
            category: "case",
            image_url: "https://m.media-amazon.com/images/I/61q3kM0A2LL._AC_SL1500_.jpg",
            description: "Compact Mid-Tower Airflow Case with RGB Fans",
            specs: ["ATX Support", "White", "Tempered Glass"]
        }
    ];

    const batch = db.batch();

    // Check if 'storage' or 'psu' or 'case' categories exist? 
    // Usually we should seed categories too if missing, but let's assume they might exist or the builder handles it.
    // The previous category seed didn't include 'storage', 'psu', 'case'. Let's add them quickly locally or safeguard.
    // I'll just add the products. Users might need to add the categories to see them in 'inventory list' filter if it relies on strict cats,
    // but the builder queries by string so it's fine.
    ...
    try {
        await batch.commit();
        alert("Success! White PC parts added.");
        if (window.loadAdminInventory) window.loadAdminInventory();
    } catch (e) {
        console.error(e);
...
function initStorefront() {
    ...
    // 1. Global Components
    if (typeof renderGlobalNavigation === 'function') renderGlobalNavigation();
    if (typeof setupCartModal === 'function') setupCartModal();

    // 2. Page Specifics
    const path = window.location.pathname;

    if ((path.includes('index.html') || path.endsWith('/')) && !path.includes('admin')) {
...
function initAdmin() {
    ...
    if (addForm) {
        addForm.addEventListener('submit', (e) => {
            ...
            if (images.length === 0) {
                images.push("https://placehold.co/400x300/000000/39ff14?text=No+Image");
            }
            ...
            db.collection("products").add({
                ...
                if (document.getElementById('p-socket')) document.getElementById('p-socket').value = ''; // Reset Socket
                // Reset Image Inputs
                const imgContainer = document.getElementById('image-inputs-container');
                if (imgContainer) {
                    imgContainer.innerHTML = `
                        <div class="flex gap-2">
                             <input type="text" class="product-image-input w-full bg-black border border-gray-800 p-2 rounded focus:border-[#00f3ff] outline-none text-sm text-white font-mono" placeholder="Image URL..." required>
                             <button type="button" class="text-red-500 px-2 opacity-50 cursor-not-allowed"><i class="fas fa-trash"></i></button>
                        </div>
                    `;
                }
                loadAdminInventory();
...
// Helper for Admin Multi-Image
window.addMoreImageInput = () => {
    const container = document.getElementById('image-inputs-container');
    if (!container) return;
    const div = document.createElement('div');
    div.className = "flex gap-2";
    div.innerHTML = `
        <input type="text" class="product-image-input w-full bg-black border border-gray-800 p-2 rounded focus:border-[#00f3ff] outline-none text-sm text-white font-mono" placeholder="Image URL...">
        <button type="button" onclick="this.parentElement.remove()" class="text-red-500 px-2 hover:text-white"><i class="fas fa-trash"></i></button>
    `;
...
function loadLatestArrivals() {
    ...
    // Default Latest
    query.orderBy("created_at", "desc").limit(10).get().then(snap => {
        let products = [];
        ...
        if (hasMore) {
            const btnHtml = `
                <div class="col-span-full flex justify-center mt-8">
                    <a href="search.html?view=all&sort=latest" class="bg-gray-900 border border-[#39ff14] text-[#39ff14] px-8 py-3 rounded-full font-bold uppercase tracking-widest hover:bg-[#39ff14] hover:text-black transition-all shadow-[0_0_20px_rgba(57,255,20,0.2)]">
                        Show All Latest Arrivals
                    </a>
                </div>
            `;
...
function loadProductDetails() {
    ...
    const container = document.getElementById('product-detail-container'); // Correct ID from product.html

    if (!id || !container) {
        // Fallback for not finding ID or Container
    ...
    db.collection('products').doc(id).get().then(doc => {
        if (!doc.exists) {
            container.innerHTML = '<div class="col-span-full text-center py-20 text-gray-500">Product not found.</div>';
            return;
        }
        ...
        // Update Breadcrumb
        const crumbName = document.getElementById('detail-name-crumb');
        if (crumbName) crumbName.innerText = p.name;

        // Update Category Breadcrumb if possible
        const crumbCat = document.getElementById('detail-category');
        ...
        container.innerHTML = `
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start mb-20">
                ...
                if (match) {
                    ...
                    const sep = match[2];
                    const val = match[3];
                    content = `<span class="text-white font-bold">${label}${sep}</span><span class="text-gray-400">${val}</span>`;
                } else {
                    ...
                    <!-- Add to Cart -->
                     ...
                     <div class="pt-6 mt-auto">
                        <div class="flex gap-4 h-14">
                             ${!isOutOfStock ? `
                             <!-- Quantity Selector -->
                             <div class="flex items-center bg-black border border-gray-800 rounded-lg overflow-hidden shrink-0">
                                 <button onclick="const q=document.getElementById('detail-qty'); if(q.value>1) q.value--" class="w-12 h-full text-gray-400 hover:text-white hover:bg-gray-900 transition-colors font-bold text-xl flex items-center justify-center">-</button>
                                 <input type="number" id="detail-qty" value="1" min="1" max="${stock}" class="bg-transparent w-12 text-center text-white font-bold text-xl outline-none appearance-none" readonly>
                                 <button onclick="const q=document.getElementById('detail-qty'); if(q.value<${stock}) q.value++" class="w-12 h-full text-gray-400 hover:text-white hover:bg-gray-900 transition-colors font-bold text-xl flex items-center justify-center">+</button>
                             </div>
                             ...
                             ` : `
                             <!-- Out of Stock -->
                             <button disabled class="w-full h-full bg-gray-900 text-gray-500 border border-gray-800 font-bold uppercase tracking-widest text-lg rounded-lg cursor-not-allowed flex items-center justify-center">
                                Product Unavailable
                             </button>
                             `}
                        </div>

                         <div class="flex items-center gap-8 mt-8 text-sm text-gray-500 uppercase font-bold tracking-wide justify-center">
            ...
            <div class="border-t border-gray-800 pt-16">
                 <div>
                     ...
                     <div class="prose prose-invert prose-lg text-gray-400 leading-relaxed max-w-4xl">
                         <p>${(p.description || "No detailed description available.").split('\n').map(line => {
                const match = line.match(/^([^:;]+)([:;])(.*)$/);
                if (match) {
                    return `<span class="text-white font-bold">${match[1]}${match[2]}</span><span>${match[3]}</span>`;
                }
                return line;
            }).join('<br>')}</p>
                     </div>
        ...
        container.innerHTML = '<div class="col-span-full text-center text-red-500">Error loading product details.</div>';
    });
}
...
window.initSearchPage = function () {
    ...
    if (termDisplay) termDisplay.innerText = query || 'Latest Arrivals';
    if (bgSearchInput && query) bgSearchInput.value = query;
...
window.renderSearchResults = function () {
    const container = document.getElementById('search-results-container') || document.getElementById('product-container');
    if (!container) return; // Prevent crash if neither exists
    ...
    if (searchResults.length === 0) {
        container.innerHTML = `
            <div class="col-span-full text-center py-20 flex flex-col items-center">
                 ...
                 <p class="text-gray-600">Check back later for stock updates or try a different category.</p>
            </div>
        `;
        return;
    }
...
if (addNavForm) {
    addNavForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const labelEl = document.getElementById('n-label');
...
window.renderGlobalNavigation = () => {
    ...
    seedStandardNavigation().then(() => {
        Promise.all([
            db.collection('navigation').orderBy('createdAt', 'asc').get(),
            db.collection('categories').get()
        ]).then(([navSnap, catSnap]) => {
            // 1. Process Categories
            const categoriesByType = {};
            catSnap.forEach(doc => {
                const c = doc.data();
                // Group by 'type' (e.g. laptop, component, accessory)
                if (!categoriesByType[c.type]) categoriesByType[c.type] = [];
                categoriesByType[c.type].push({ id: doc.id, ...c });
            });

            // 2. Define standard links
            const links = [
                { label: 'Home', url: 'index.html' },
                { label: 'Custom PC Builder', url: 'builder.html' }
            ];
            ...
            links.forEach(link => {
                ...
                // Check for Dropdown Data
                const cats = link.type ? (categoriesByType[link.type] || []) : [];

                // Base Classes
                const baseClass = "px-4 h-10 flex items-center rounded transition-all uppercase text-sm tracking-wider font-bold relative group cursor-pointer";
                const activeClass = isActive ? 'text-[#39ff14]' : 'text-gray-300 hover:text-white';
                ...
                if (cats.length > 0) {
                    ...
                    html += `
                        <div class="${baseClass} ${activeClass}">
                            <a href="${link.url}" class="flex items-center gap-1">
                                ${link.label} <i class="fas fa-chevron-down text-[10px] transform group-hover:rotate-180 transition-transform"></i>
                            </a>
                            
                            <!-- Dropdown Menu -->
                            <div class="absolute left-0 top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 min-w-[220px]">
                                <div class="bg-[#0a0a0a] border border-gray-800 rounded-lg shadow-[0_10px_40px_rgba(0,0,0,0.9)] overflow-hidden flex flex-col py-1">
                                    ${cats.map(c => `
                                        <a href="category.html?id=${c.id}" class="px-4 py-3 flex items-center gap-3 text-gray-400 hover:text-white hover:bg-white/5 transition-colors border-b border-gray-800/50 last:border-0 group/item">
                                            <i class="${c.icon || 'fas fa-microchip'} text-[#39ff14] w-5 text-center group-hover/item:scale-110 transition-transform"></i>
                                            <span class="text-xs font-bold uppercase tracking-wide">${c.name}</span>
                                        </a>
                                    `).join('')}
                                </div>
                            </div>
                    ...
                    // STANDARD LINK
                    html += `<a href="${link.url}" class="${baseClass} ${activeClass}">${link.label}</a>`;
                }
            });
...
// 7. Inject Mobile Bottom Nav
window.injectMobileNav = () => {
    if (document.getElementById('mobile-bottom-nav')) return;

    const nav = document.createElement('div');
    nav.id = 'mobile-bottom-nav';
    nav.className = "md:hidden fixed bottom-0 left-0 right-0 z-50 bg-gray-900/90 backdrop-blur-lg border-t border-[#39ff14]/30 pb-[env(safe-area-inset-bottom)] padding-bottom: var(--safe-area-bottom)";
    nav.innerHTML = `
        <div class="flex justify-around items-center h-16 px-2">
            <a href="index.html" class="flex flex-col items-center justify-center text-gray-400 hover:text-[#39ff14] active:text-[#39ff14] transition-colors w-full h-full">
                <i class="fas fa-home text-xl mb-1"></i>
                <span class="text-[10px] font-bold uppercase">Home</span>
            </a>
            <a href="search.html" class="flex flex-col items-center justify-center text-gray-400 hover:text-[#39ff14] active:text-[#39ff14] transition-colors w-full h-full">
                <i class="fas fa-search text-xl mb-1"></i>
                <span class="text-[10px] font-bold uppercase">Search</span>
            </a>
            <div class="relative flex flex-col items-center justify-center text-gray-400 hover:text-[#39ff14] active:text-[#39ff14] transition-colors w-full h-full cursor-pointer" onclick="window.toggleCart()">
                <div class="relative">
                    <i class="fas fa-shopping-cart text-xl mb-1"></i>
                    <span id="mobile-cart-count" class="absolute -top-2 -right-2 bg-[#ff00ff] text-white text-[9px] font-bold w-4 h-4 flex items-center justify-center rounded-full border border-black">0</span>
                </div>
                <span class="text-[10px] font-bold uppercase">Cart</span>
            </div>
            <a href="admin.html" class="flex flex-col items-center justify-center text-gray-400 hover:text-[#39ff14] active:text-[#39ff14] transition-colors w-full h-full">
    ...
    const updateMobileCount = () => {
        const desktopCnt = document.getElementById('cart-count');
        const mobileCnt = document.getElementById('mobile-cart-count');
        if (desktopCnt && mobileCnt) mobileCnt.innerText = desktopCnt.innerText;
    };

    // Initial Sync
    setTimeout(updateMobileCount, 1000);

    // Observe changes to desktop cart count to sync mobile
    const deskCount = document.getElementById('cart-count');
    if (deskCount) {
        const observer = new MutationObserver(updateMobileCount);
        observer.observe(deskCount, { childList: true, subtree: true, characterData: true });
    }
...
window.initCategoryPage = function () {
    ...
    const sidebarList = document.getElementById('dynamic-sidebar-categories');

    if (catId) {
        // --- SINGLE CATEGORY MODE ---
        if (sidebarContainer) sidebarContainer.classList.add('hidden');

        // Fetch ALL products to filter case-insensitively (Client-side Fix)
        // (Added Debug Logs & Trim)
        db.collection('products').get().then(snap => {
            console.log(`[DEBUG] Fetching for category ID: "${catId}"`);
            const products = [];

            // TRIM and LOWERCASE the target from URL
            const target = catId ? catId.trim().toLowerCase() : '';
            console.log(`[DEBUG] Target processed: "${target}"`);

            snap.forEach(doc => {
                const d = doc.data();
                // Handle missing or whitespace-laden categories
                const pCat = d.category ? d.category.trim().toLowerCase() : '';
            ...
            console.log(`[DEBUG] Match count: ${products.length}`);
...
window.renderGlobalFooter = () => {
    ...
    const config = {
        brandName: "TECHZONE",
        desc: "Follow us on social media for updates and offers!",
        socials: [
    ...
    footer.innerHTML = `
        <div class="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            <!-- Brand -->
            <div>
                <h2 class="text-2xl font-black text-[#39ff14] mb-4 tracking-tighter italic">${config.brandName}</h2>
                <p class="text-gray-500 mb-6 max-w-xs">${config.desc}</p>
                <div class="flex space-x-4">
                    ${config.socials.map(s => `
                        <a href="${s.url}" class="w-10 h-10 rounded-full bg-gray-900 flex items-center justify-center text-gray-400 hover:bg-[#39ff14] hover:text-black transition-all">
                            <i class="${s.icon}"></i>
                        </a>
                    `).join('')}
                </div>
            ...
            <!-- Contact -->
            <div>
                <h3 class="text-white font-bold uppercase tracking-wider mb-6">Contact</h3>
                <ul class="space-y-4 text-gray-400 text-sm">
                    <li>
                        <a href="${config.contact.mapLink}" target="_blank" class="flex items-start gap-3 hover:text-[#39ff14] transition-colors group">
                            <i class="fas fa-map-marker-alt mt-1 group-hover:animate-bounce"></i>
                            <span>${config.contact.address}</span>
                        </a>
                    </li>
                    <li>
                        <a href="${config.contact.whatsapp}" target="_blank" class="flex items-center gap-3 hover:text-[#39ff14] transition-colors">
        ...
        <div class="max-w-7xl mx-auto px-4 pt-8 border-t border-gray-900 text-center text-gray-600 text-xs">
